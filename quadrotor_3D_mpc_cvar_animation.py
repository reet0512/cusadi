...
psi_log   = torch.zeros((STEPS, PLOTS), device=device)
...
# ================================================================
# 8. Plot sample trajectories (x, y, z separately)
# ================================================================
px = px_log.cpu().numpy()
py = py_log.cpu().numpy()
pz = pz_log.cpu().numpy()
...
# ================================================================
# 9. 3D animation of one quadrotor trajectory
# ================================================================
print("Building 3D animation...")

# Use the first logged quadrotor
idx = 0
x_traj = px_log[:, idx].cpu().numpy()
y_traj = py_log[:, idx].cpu().numpy()
z_traj = pz_log[:, idx].cpu().numpy()
phi_traj   = phi_log[:, idx].cpu().numpy()
theta_traj = theta_log[:, idx].cpu().numpy()
psi_traj   = psi_log[:, idx].cpu().numpy()

# Simple quadrotor cross in body frame (arms length)
arm_len = 0.15
...
# path line
path_line, = ax3d.plot([], [], [], "k-", lw=1.5, label="trajectory")

# body arms
arm_x_line, = ax3d.plot([], [], [], "r-", lw=2, label="arm x")
arm_y_line, = ax3d.plot([], [], [], "b-", lw=2, label="arm y")

# set axis limits from data
pad = 0.3
x_min, x_max = x_traj.min() - pad, x_traj.max() + pad
y_min, y_max = y_traj.min() - pad, y_traj.max() + pad
z_min_ax, z_max_ax = z_traj.min() - pad, z_traj.max() + pad

ax3d.set_xlim([x_min, x_max])
ax3d.set_ylim([y_min, y_max])
ax3d.set_zlim([z_min_ax, z_max_ax])
ax3d.set_xlabel("x (m)")
ax3d.set_ylabel("y (m)")
ax3d.set_zlabel("z (m)")
ax3d.set_title("3D Quadrotor — CVaR MPC")
...
def init_anim():
    ...
    return path_line, arm_x_line, arm_y_line


def update_anim(frame):
    # trajectory so far
    path_line.set_data(x_traj[:frame+1], y_traj[:frame+1])
    path_line.set_3d_properties(z_traj[:frame+1])

    # current pose
    pos = np.array([x_traj[frame], y_traj[frame], z_traj[frame]])
    R   = euler_to_rot(phi_traj[frame], theta_traj[frame], psi_traj[frame])

    pts_world = (R @ body_points.T).T + pos  # (4,3)
    # first pair: +/- x arms
    arm_x_line.set_data([pts_world[0,0], pts_world[1,0]],
                        [pts_world[0,1], pts_world[1,1]])
    arm_x_line.set_3d_properties([pts_world[0,2], pts_world[1,2]])
    # second pair: +/- y arms
    arm_y_line.set_data([pts_world[2,0], pts_world[3,0]],
                        [pts_world[2,1], pts_world[3,1]])
    arm_y_line.set_3d_properties([pts_world[2,2], pts_world[3,2]])
...
# Save to file (if ffmpeg is installed) and show
anim_path = "results/quadrotor_3d_cvar_anim.mp4"
try:
    anim.save(anim_path, fps=30, extra_args=["-vcodec", "libx264"])
    print(f"Saved 3D animation to {anim_path}")
except Exception as e:
    print("Could not save animation (ffmpeg missing?) — showing inline only.")
    print("Error:", e)
